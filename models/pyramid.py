...
- build_gaussian_pyramid: [full-res, half-res, quarter-res, ...] -- these are the
  "zoom levels" the multi-scale model corrects independently.
...
- build_pyramid / reconstruct: the Laplacian pyramid (detail bands + coarse base).
  ...
  reconstruct(build_pyramid(x)) == x -- because reconstruction is just algebraic
  inversion of the construction step. If test_pyramid.py doesn't confirm that,
  something is wrong here, not in your training loop.
"""
...
def build_gaussian_pyramid(img, levels):
    ...
    for _ in range(levels):
        gauss.append(F.avg_pool2d(gauss[-1], 2))
    return gauss
