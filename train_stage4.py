"""
Stage 4: Multi-scale training script.

Run test_pyramid.py FIRST and confirm it passes before running anything here.

1. OVERFIT TEST:
   python train_stage4.py --root /path/to/LOLdataset --overfit_n 8 --epochs 200

2. FULL TRAINING:
   python train_stage4.py --root /path/to/LOLdataset --epochs 100

Loss = pixel loss on the fused output + Retinex recon/smoothness losses averaged
across all scales (same physics terms as Stage 3, just applied at every scale now).

Outputs go to --out_dir:
    runs/stage4/checkpoints/epoch_N.pt
    runs/stage4/samples/epoch_N.png        -- rows: low / fused prediction / high
    runs/stage4/samples/epoch_N_scales.png -- each scale's corrected output, upsampled
"""
...
def main(args):
    ...
    model = MultiScaleExposureNet(channels=args.channels, iters=args.iters, levels=args.levels).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for low, high, names in train_loader:
            ...
            fused, upsampled, diagnostics = model(low)
            ...
            last_batch = (low, high, fused, upsampled)
...
def save_samples(last_batch, epoch, sample_dir, n=4):
    low, high, fused, upsampled = last_batch
    n = min(n, low.size(0))
    ...
    # each scale's own corrected output (upsampled to full res) -- check these look
    # progressively coarser/smoother from fine to coarse scale, not identical to each other
    scale_rows = torch.cat([s[:n].clamp(0, 1).cpu() for s in upsampled], dim=0)
    scale_grid = vutils.make_grid(scale_rows, nrow=n)
    vutils.save_image(scale_grid, os.path.join(sample_dir, f"epoch_{epoch + 1}_scales.png"))

    print(f"saved samples for epoch {epoch + 1} (fused grid + per-scale grid)")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--levels", type=int, default=2, help="Extra downsampled scales beyond full-res (2 -> 3 scales total)")
    ...
    parser.add_argument("--out_dir", type=str, default="runs/stage4")
